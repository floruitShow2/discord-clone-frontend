...
const CellGroup: FunctionComponent<BaseProps> = (props) => {
    ...
    const genTextCell = (cell: CellConfig, index: number) => {
        ...
        return <li key={index} className={cellBaseCls}>
            <div className='w-full flex items-center justify-between'>
                <h4 className='text-sm text-black dark:text-white'>{label}</h4>
                <div className='text-xs text-light-l'>
                    {
                        isEditing
                            ? <Input value={value} size='mini' onChange={handleInputChange} onBlur={() => setIsEditing(false)} />
                            : <span>{value}</span>
                    }
                    {(allowEdit && !isEditing) && <IconPen className='ml-1 cursor-pointer hover:text-blue-500' onClick={() => setIsEditing(true)} />}
                </div>
            </div>
    ...
    return <ul className='w-full flex flex-col items-center justify-start border border-primary-b rounded-md'>
        {
